    ...
    def _load_metric_descriptions(self) -> Dict:
        ...
        return {
            ...
            'innovation': {
                ...
                'growth_score': {
                    'name': 'Growth Score Metrics',
                    'description': 'Measures platform success in early topic development using a time-weighted scoring system. For each cluster, considers all markets created within 2 weeks of the first market. The first market receives maximum weight (100), with subsequent markets receiving decreasing weight via a dropoff function: score = 105/(1 + 0.05*hours) - 5, with a minimum of 0. Platform weights are summed within each cluster, then converted to proportional scores (0-1) reflecting each platform\'s share of early development activity. The market-weighted variants multiply growth weights by individual market scores to reward high-quality early markets.',
                    'interpretation': 'Identifies platforms that excel at recognizing and rapidly developing promising new topics, avoiding the "winner takes all" bias of simple founder metrics. Unlike founder metrics that only reward the first market, growth scores account for sustained early investment and development speed. High scores indicate platforms with strong market instincts that quickly identify and capitalize on emerging opportunities. The rapid dropoff function (weight halves every ~14 hours) rewards true early movers while still giving credit to fast followers. Market-weighted variants additionally reward platforms that create valuable markets during growth phases, not just numerous ones. This metric captures the crucial early development window when topics transition from single ideas to established categories.',
                    'variations': ['growth_score_total', 'growth_score_weighted', 'growth_score_clusters_participated', 'growth_score_mean', 'growth_score_market_weighted_total', 'growth_score_market_weighted_weighted', 'growth_score_market_weighted_clusters_participated', 'growth_score_market_weighted_mean']
                }
            },
            'competition': {
                'topic_flow': {
