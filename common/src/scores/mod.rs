...
use crate::probability::{CriterionProbability, DailyProbability};
...
impl AbsoluteScoreType {
    ...
    pub fn get_criterion_prob(&self, criteron_probs: &[CriterionProbability]) -> Option<f32> {
        match self {
            AbsoluteScoreType::BrierAverage
            | AbsoluteScoreType::LogarithmicAverage
            | AbsoluteScoreType::SphericalAverage => {
    ...
    pub fn get_score(
        &self,
        market: &Market,
        criteron_probs: &[CriterionProbability],
    ) -> Option<f32> {
        match self {
            AbsoluteScoreType::BrierAverage
            | AbsoluteScoreType::BrierMidpoint
...
pub fn calculate_absolute_scores(
    markets: &[Market],
    criterion_probs: Vec<CriterionProbability>,
) -> Result<Vec<MarketScore>> {
...
pub fn calculate_relative_scores(
    ...
    for question in questions {
        ...
        let question_probs: Vec<DailyProbability> = probs
            .iter()
