...
use crate::{DailyProbabilityPoint, MarketWithProbs};
...
pub fn score_market(
    ...
    let consensus_resolution = if let Some(first_market) = outcome_links.first() {
        match first_market.question_invert {
            true => 1.0 - first_market.outcome.resolution.into(),
            false => first_market.outcome.resolution,
        }
    } else {
        return Err(anyhow!("No markets provided"));
    };
    for link in outcome_links {
        let market_resolution = match link.question_invert {
            true => 1.0 - link.outcome.resolution.into(),
            alse => link.outcome.resolution,
        };
        if market_resolution != consensus_resolution {
            return Err(anyhow!(
                "Outcome {} resolved differently than consensus",
                link.outcome.id
            ));
        }
    }
    ...
    let markets_with_probs: Vec<MarketWithProbs> = outcome_links
        .iter()
        .map(|link| {
            ...
            if daily_probs.is_empty() {
                if prob_count_unfiltered == 0 {
                    return Err(anyhow!(
                        "No probabilities found for market {}",
                        link.outcome.id
                    ));
                } else {
                    return Err(anyhow!(
    ...
    let mut relative_scores: HashMap<String, Vec<f64>> = outcome_links
        .iter()
    ...
    for day in days {
        let mut daily_market_absolute_scores = HashMap::with_capacity(outcome_links.len());
        for market in outcome_links {
            ...
            let score = score_type.get_score(prediction, consensus_resolution);
