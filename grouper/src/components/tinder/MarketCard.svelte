...
<div class="bg-crust p-4 rounded-lg border {isMain ? 'border-green' : 'border-overlay0'} shadow-sm mb-3">
  <div class="flex justify-between items-start mb-2">
    <div class="flex items-center">
      <div class="px-2 py-1 rounded bg-overlay0 text-crust text-xs font-medium">
        {market.platform_name || market.platform_slug}
      </div>
      {#if market.category_name}
        <div class="ml-2 px-2 py-1 rounded bg-blue text-crust text-xs font-medium">
  ...
  {#if market.description && market.description.length > 0}
    <div class="text-sm mb-3 text-text/80 border-t border-overlay0 pt-2 line-clamp-2">
      {market.description}
    </div>
  {/if}
