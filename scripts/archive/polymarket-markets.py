...
def analyze_markets(jsonl_file):
    ...
    with open(jsonl_file, "r") as f:
        for line in f:
            ...
            if abs(token_prices_sum - Decimal("1")) > Decimal(
                "0.0001"
            ...
            # Timestamp analysis
            if (
                "end_date_iso" in market
                and market["end_date_iso"]
                and "prices_history" in data
                and data["prices_history"]
            ):
                end_date = parse_iso(market["end_date_iso"])
                last_price_time = datetime.fromtimestamp(
                    data["prices_history"][-1]["t"], tz=utc
                )
    ...
    for (active, closed), count in active_closed_stats.items():
        print(
            f"Active: {active}, Closed: {closed}: {count} markets ({count / total_markets * 100:.1f}%)"
        )

    print(
        f"end_date_iso present in {end_date_present}/{total_markets} markets ({end_date_present / total_markets * 100:.1f}%)"
    )
    ...
    print(
        f"Yes/No markets with Yes first: {yes_first_in_yes_no}/{yes_no_markets} "
